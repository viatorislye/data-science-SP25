...
-   You chose a correct value of `stat(x, y)` when `x, y` is *outside* the circle. Why did you choose this value?
    -   I chose x =1 & x=1 as it is the farther point in our graph and our circle is centered around 0,0.
-   You chose a correct value of `stat(x, y)` when `x, y` is *inside* the circle. Why did you choose this value?
    -   I chose x =.5 & x=.5 as it falls in the center of our graph and is clearly in the orange zone. This is because sqrt(.5^2^  + .5\^2 ) is .7071 which is shorter than the radius of 1.
...
-   How closely do your bootstrap CI and CLT CI agree?
    -   Their variation happens is the ten thousands place so there are very close.
-   Comment on the width of your CI(s). Would your estimate of $\pi$ be good enough for roughly estimating an area (e.g., to buy enough paint for an art project)? Would your estimate of $\pi$ be good enough for precisely calculating a trajectory (e.g., sending a rocket into orbit)?
    -   (Good enough as a rough estimate?)
        -   It's good enough with paint especially if you end up a little short, you can add some water to the paint to cover the unpainted area.
    -   (Good enough as a precise estimate?)
        -   Absolutely not good enough for trajectory because half a degree can change the entire trajectory of a rocket
-   What would be a *valid* way to make your CI more narrow?
    -   Taking more samples to give the CI more reference points in order to give a more accurate number.
