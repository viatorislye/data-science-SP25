...
- Which model is *most accurate*? Which is *least accurate*?
  - The most accurate model is Cheat. The least accurate model is non
    physical
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - Avg_t is an output and is the Plane-averaged fluid temperature.
    T_norm is the normalized fluid temperature rise thus it is
    determined from the avg_T. This is want makes it a good predictor.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No because avg_T is an output and T_norm is calculated from it and
    normalized but it is very hard to un normalized something.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - Idx is metadata variable and marks the simulation run and that’s it.
    So there is not physical meaning.
...
```
    ...
    ## 
    ## Coefficients:
    ## (Intercept)            L            W          U_0          N_p          k_f  
    ##   4.882e+00    2.570e+00   -4.442e+01   -1.682e-01   -4.017e-07   -2.280e+00  
    ##         T_f  
    ##  -4.783e-03
...
- Which columns are excluded in the model formula above? What categories
  ...
  1.  Excludes: x (spiarl), idx (meta data) and all the variables below
      ...
      2.  This variable either don’t have physical meaning, are input
          deemed statistically insignificant or are outputs.
...
- How do these standard deviations relate to the regression coefficients
  for `x` and `T_f`?
  - It could be that the SD for x is small as the regression coefficient
    was zero and the opposite is true for T_f. I am unsure though.
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - That interpreting statistical significance by regression
    coefficients is an approximate but there will still be significant
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - I think the model q4 is more accurate as it generally follows the
    trend better than x only. However, I’m having a hard time reading
    the graph as there is a lot of overlap
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*?
  - I think the model q4 is more confident as the some short error bars.
    Again I’m having a hard time seeing which one is narrower.
...
- How much do you trust your model? Why?
  - I don’t trust it that much solely because I don’t know if I did the
    operation correctly.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - I picked prediction as prediction intervals are meant to capture
    *new observations* which you need when you’re designing something
- What fraction of validation cases lie within the intervals you
  ...
  - At a pr_level 80%, the mean(within_interval\$within) states that 65%
    will be in the bounds so it’s not perfect but good enough depending
    on application
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - The T_norm is -.88 to 2.67 for a 80% prediction interval. So the
    design team should interval of -.13 to 2 to have a range that will
    be in this bounds. (I pick a smaller range but no specific numbers)
...
- \[1\] Jofre, del Rosario (\<-Look it’s ZDR!!!), and Iaccarino
  “Data-driven dimensional analysis of heat transfer in irradiated
  particle-laden turbulent flow” (2020) *International Journal of
  Multiphase Flow*,
